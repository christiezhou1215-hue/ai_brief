...
- 聚合 130 个数据源，覆盖科技媒体、官方实验室、学术研究、开发者社区和国际一手来源
...
也可通过 `AI_API_KEY`、`AI_BASE_URL` 和 `AI_MODEL` 使用其他 OpenAI Chat Completions 兼容接口。模型未配置或调用失败时，接口会使用本地归纳或翻译降级方案，避免页面中断。
