...
type NewsItem = { id: string; title: string; source: string; sourceMark: string; publishedAt: string; category: string; level: "重要" | "关注" | "一般"; score: number; whyImportant: string; summary: string; imageUrl?: string; tags: string[]; url: string; related: number };
...
function importanceFor(item: Pick<NewsItem, "title" | "summary" | "source" | "category" | "related">) {
  ...
  return { score, level, whyImportant };
}
